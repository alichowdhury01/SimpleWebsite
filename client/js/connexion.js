     ...
     for(let uneConnexion of connexion){
        if(uneConnexion.courriel == courriel && uneConnexion.pass == pass){
            if(uneConnexion.statut == "A") {
                ...
                window.open('client/pages/commandes.html');//rediriger vers la page membre.html
            } else {
                msgErr="Impossible de vous connecter, ce compte a été désactiver. Contactez l'administrateur.";
                break;
            }
        }
     }
